...
void client() {
    ...
    while(run) {
        ...
        set_time();
        printf("%s New client has arrived. Hairstyle number: %d, time: %fs\n", time_msg, service_number+1, time/1000.0);
        ...
        if (times[last_client+1] == -1) {
            ...
            semop(semDesc, &sem_buf, 1);
            set_time();
            printf("%s Client is in the waiting room\n", time_msg);
        }
        else {
            set_time();
            printf("%s No empty place in waiting room, client leaves\n", time_msg);
        }
        shmdt(times);
        sem_buf.sem_num = TIMES_SEM_ID;
        sem_buf.sem_op = 1;
...
void hairdresser(int id) {
    int *times;
    set_time();
    printf("%s Hairdresser %d is ready to work\n", time_msg, id);
    while(run) {
        ...
        usleep(time);
        set_time();
        printf("%s Hairdresser %d has finished hairstyle\n", time_msg, id);
        sem_buf.sem_num = CHAIR_SEM_ID;
        sem_buf.sem_op = 1;
        semop(semDesc, &sem_buf, 1);
    }
...
int main(int argc, char** argv) {
    if (argc < 5) {
        fprintf(stderr, "Wrong input (hairdressers|seats|waiting room|hairstyles)\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (M < N) {
        fprintf(stderr, "Number of hairdressers must be greater than available seats\n");
        exit(EXIT_FAILURE);
    }
    ...
    printf("AVAILABLE HAIRSTYLES\n");
    for (int i = 0; i < F; i++) {
        service_times[i] = 3000 + random() % 5000;
        printf("Hairstyle number: %d, time to complete: %fs\n", i+1, service_times[i]/1000.0);
    }
    printf("\n");

    key = ftok("barber.c", 1 + rand()%255);
